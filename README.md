# 🧬 Scientific Computing Portfolio

> **7+ Years Contributing to Global Scientific Research Through Distributed Computing**

[![BOINC](https://img.shields.io/badge/BOINC-10.2M_Cobblestones-blue?style=for-the-badge&logo=atom)](https://boinc.berkeley.edu/)
[![Folding@home](https://img.shields.io/badge/Folding@home-3.8M_Points-green?style=for-the-badge&logo=moleculer)](https://foldingathome.org/)
[![WCG](https://img.shields.io/badge/World_Community_Grid-Active-orange?style=for-the-badge&logo=ibm)](https://www.worldcommunitygrid.org/)

I contribute idle computing power across multiple platforms to accelerate scientific research in medicine, astronomy, mathematics, and physics.

---

## 🏆 Portfolio Overview

**Total Contribution:**
- ⚡ **10.2+ Million BOINC Cobblestones** across 10+ global projects
- 💻 **3.8+ Million Folding@home Points** (Rank #216,493 globally)
- 🌍 **World Community Grid** active contributor
- 🔬 **8.84 Quintillion** floating-point operations for science
- 📅 **Active since 2018** - 7+ years of continuous contribution

**Research Impact Areas:**
🧬 Drug Discovery • ⚕️ Cancer Research • 🧠 Neurological Disorders • 🦠 Infectious Diseases • 🔭 Astronomy • 📐 Mathematics • 💊 Protein Folding

---

## 🔬 BOINC Network Projects

I contribute to the Berkeley Open Infrastructure for Network Computing (BOINC), the world's largest volunteer computing network.

### 📜 [Certificate of Computation](https://boinc.bakerlab.org/rosetta/cert_all.php)

**Total Contribution: 10,232,667 Cobblestones**

### Active Projects:

| Project | Cobblestones | Research Area | Active Since |
|---------|--------------|---------------|--------------|
| **GPUGRID** | 6,450,000 | 💊 Drug Discovery & Molecular Dynamics | Apr 2019 |
| **Amicable Numbers** | 2,296,961 | 📐 Number Theory & Mathematics | Oct 2019 |
| **SiDock@home** | 482,797 | 🧬 Drug Docking Simulations | Apr 2025 |
| **Rosetta@home** | 439,496 | 🧬 Protein Structure Prediction | Jun 2018 |
| **PrimeGrid** | 224,437 | 🔢 Prime Number Research | Sep 2019 |
| **MilkyWay@home** | 124,111 | 🌌 Milky Way Galaxy Modeling | Oct 2019 |
| **yoyo@home** | 101,050 | 🧮 Mathematical Research | Aug 2018 |
| **Asteroids@home** | 75,840 | ☄️ Asteroid Shape Modeling | Aug 2018 |
| **SETI@home** | 21,928 | 👽 Search for Extraterrestrial Intelligence | Aug 2018 |
| **Ramanujan Machine** | 16,011 | 🔬 Mathematical Constants | Mar 2025 |

### What This Means:

**GPUGRID (6.4M Cobblestones):**
- GPU-accelerated molecular dynamics simulations
- Drug discovery for diseases including cancer and COVID-19
- Protein-ligand interaction studies

**Amicable Numbers (2.3M Cobblestones):**
- Pure mathematics research
- Number theory and computational mathematics
- Contributing to mathematical knowledge expansion

**SiDock@home (482k Cobblestones):**
- Drug docking simulations for disease treatment
- Identifying potential drug candidates
- COVID-19 therapeutic research

**Rosetta@home (439k Cobblestones):**
- Protein structure prediction
- Disease understanding through protein analysis
- Crucial for drug design and disease treatment

**And 6 more active projects...**

---

<!-- WCG_STATS_START -->
## 📊 Live Platform Statistics

*Loading dynamic statistics from Folding@home and World Community Grid APIs...*

*This section updates automatically daily via GitHub Actions.*

<!-- WCG_STATS_END -->

---

## 🌟 Why This Matters

### The Power of Distributed Computing

> "Research that would take centuries on a single computer can be completed in months through volunteer computing networks."

**My Contribution:**
- 🔬 **8.84 quintillion** floating-point operations donated to science
- ⏱️ **Thousands of hours** of GPU/CPU time
- 🌍 **10+ research projects** simultaneously supported
- 📈 **7+ years** of consistent contribution

**Real-World Impact:**
- Drug candidates discovered for COVID-19, cancer, and Alzheimer's
- Protein structures solved for disease research
- Mathematical knowledge expanded
- Astronomical phenomena modeled
- Scientific breakthroughs accelerated

---

## 🔧 Technical Implementation

This repository demonstrates advanced DevOps and automation skills:

### Architecture

```
┌─────────────────────────────────────────┐
│        GitHub Actions (Cron: Daily)     │
└──────────────┬──────────────────────────┘
               │
       ┌───────┴───────┐
       │               │
       ▼               ▼
┌─────────────┐ ┌─────────────┐
│   F@H API   │ │   WCG API   │
└──────┬──────┘ └──────┬──────┘
       │               │
       └───────┬───────┘
               ▼
      ┌─────────────────┐
      │  Python Script  │
      │  Data Processing│
      └────────┬────────┘
               │
       ┌───────┴───────┐
       │               │
       ▼               ▼
┌─────────────┐ ┌─────────────┐
│ README.md   │ │ JSON Data   │
│ (Auto-updated)│ │  Storage  │
└─────────────┘ └─────────────┘
```

### Tech Stack

- **Python 3.11** - Multi-platform API integration
- **GitHub Actions** - Automated daily statistics updates
- **REST APIs** - Folding@home & World Community Grid
- **Markdown Badges** - Professional visual display
- **Git Automation** - Zero-touch deployment

### Features

✅ Fully automated daily updates  
✅ Multi-platform data aggregation  
✅ Real-time API statistics  
✅ Production-ready error handling  
✅ Zero manual intervention required  
✅ Professional documentation  

---

## 📁 Repository Structure

```
scientific-computing-dashboard/
├── .github/workflows/
│   └── update_scientific_stats.yml    # Daily automation
├── scripts/
│   └── fetch_stats.py                 # Multi-platform fetcher
├── data/
│   └── wcg-stats.json                 # Live statistics
├── docs/
│   └── index.html                     # Optional web dashboard
└── README.md                           # This file (auto-updated)
```

---

## 🚀 Getting Started

Want to contribute to scientific research too?

### Join These Platforms:

**🔬 BOINC (Multiple Projects):**
1. Download [BOINC Client](https://boinc.berkeley.edu/download.php)
2. Choose research projects
3. Let your computer solve problems while idle

**💻 Folding@home (Protein Folding):**
1. Visit [foldingathome.org](https://foldingathome.org/)
2. Download client
3. Start folding proteins for disease research

**🌍 World Community Grid (Medical Research):**
1. Visit [worldcommunitygrid.org](https://www.worldcommunitygrid.org/)
2. Create account
3. Download BOINC and join WCG projects

---

## 📊 Impact Metrics

**Since 2018:**
- 🎯 **10+ Projects** supported simultaneously
- ⚡ **10M+ Cobblestones** earned across BOINC network
- 💻 **3.8M+ Points** contributed to Folding@home
- 🧬 **Multiple diseases** researched (COVID-19, Cancer, Alzheimer's, etc.)
- 🌍 **Global impact** through distributed computing

**Categories of Research:**
- 💊 **Pharmaceutical Development** (Drug discovery, protein docking)
- 🧬 **Molecular Biology** (Protein folding, structure prediction)
- 🦠 **Infectious Diseases** (COVID-19, pandemic preparedness)
- ⚕️ **Cancer Research** (Molecular dynamics, treatment discovery)
- 🧠 **Neurological Disorders** (Alzheimer's, Parkinson's)
- 🔭 **Astrophysics** (Galaxy modeling, asteroid tracking)
- 📐 **Pure Mathematics** (Number theory, computational math)

---

## 🤝 Connect

**My Profiles:**
- 💻 Folding@home: [HenningSarrus](https://stats.foldingathome.org/donor/HenningSarrus)
- 🌍 World Community Grid: [henning.sarrus](https://www.worldcommunitygrid.org/stat/viewMemberInfo.do?userName=henning.sarrus)
- 🔬 BOINC: [Certificate of Computation](https://boinc.bakerlab.org/rosetta/cert_all.php)

---

## 📜 License

This automation implementation is open source. MIT License.

---

**🔄 Last Update:** Automated via GitHub Actions  
**🤖 Update Frequency:** Daily at 00:00 UTC  
**📊 Data Sources:** Live APIs from F@H, WCG, and BOINC projects

---

*Contributing to science, one computation at a time.* 🧬
